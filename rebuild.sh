...
psql -f "./src/001a_health_tables.sql" -d $DATABASE_NAME --single-transaction  # NEW
psql -f "./src/002_triggers.sql" -d $DATABASE_NAME --single-transaction
psql -f "./src/002a_health_triggers.sql" -d $DATABASE_NAME --single-transaction  # NEW
...
psql -f "./src/007_health_functions.sql" -d $DATABASE_NAME --single-transaction  # NEW
psql -f "./src/008_monitoring_views.sql" -d $DATABASE_NAME --single-transaction  # NEW
